...
def candle(i: int, close: float = 100.0, low: float | None = None,
           high: float | None = None, interval: str = "1h") -> Candle:
    return Candle(
...
class EnterOnce(Strategy):
    def __init__(self, at_bar: int = 30, stop_pct: float = 0.05):
...
def make_runtime(strategy: Strategy, captured: Captured, controls=None) -> TradingRuntime:
    ...
    return TradingRuntime(
        broker=PaperBroker(PaperAccount.with_starting_balance("USDT", D("10000")), CostModel()),
        strategies=[strategy],
        risk=BasicRiskEngine(RiskConfig(min_confidence=0.5)),
...
class TestPipeline:
    async def test_signal_to_execution_to_position(self):
        captured = Captured()
        runtime = make_runtime(EnterOnce(at_bar=30), captured)
        await feed(runtime, [candle(i) for i in range(35)])
    ...
    async def test_stop_loss_closes_position(self):
        captured = Captured()
        runtime = make_runtime(EnterOnce(at_bar=30, stop_pct=0.05), captured)
        candles = [candle(i) for i in range(35)]
        candles.append(candle(35, close=94.0, low=90.0))   # pierces the 95 stop
    ...
    async def test_duplicate_candles_are_idempotent(self):
        ...
        runtime = make_runtime(EnterOnce(at_bar=30), captured)
        candles = [candle(i) for i in range(35)]
        await feed(runtime, candles + candles[-5:])        # replayed events
        assert len(captured.executions) == 1               # no duplicate orders
    ...
    async def test_daily_loss_halt_blocks_new_entries(self):
        ...
        runtime._risk_state.daily_realized_pnl = -1000.0   # 10% loss — beyond 2% limit
        runtime.seed_equity(10_000.0)
        await feed(runtime, [candle(i) for i in range(15)])
...
class TestEmergencyStop:
    ...
    async def test_paused_controls_block_trading(self):
        ...
        captured = Captured()
        runtime = make_runtime(EnterOnce(at_bar=30), captured, controls=controls)
        await feed(runtime, [candle(i) for i in range(35)])
        assert not captured.executions                     # nothing traded while paused